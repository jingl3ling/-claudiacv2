...
async function selectScenario(emo) {
  ...
  // Adjust polling rate based on whether we're in realtime mode
  isRealtimeMode = emo.mode === "realtime";
  applyPollingRates();
...
// connect / boot
// ----------------------------------------------------------------------
let isRealtimeMode = false;  // when true, poll faster

function applyPollingRates() {
  if (analyzeTimer) window.clearInterval(analyzeTimer);
  if (waveFetchTimer) window.clearInterval(waveFetchTimer);
  const analyzeMs = isRealtimeMode ? 700 : 1200;
  const waveMs = isRealtimeMode ? 700 : 1500;
  analyzeTimer = window.setInterval(fetchAnalyzeOnce, analyzeMs);
  waveFetchTimer = window.setInterval(fetchWaveformOnce, waveMs);
}
